...
#define k_enter_key 3
#define k_return_key 13
#define k_escape_key 27

#define r_ALRT_about 128
#define r_DITL_about 128
...
static void flash_button(DialogPtr dialog, short item) {
	long ticks;
	Handle handle;
	Rect rect;
	short kind;
	...
	GetDItem(dialog, item, &kind, &handle, &rect);
	switch (kind) {
...
static void draw_default_button_outline(DialogPtr dialog) {
	GrafPtr saved_port;
	Handle handle;
	Rect rect;
	short kind;
...
static Boolean is_ok_key_event(EventRecord *event) {
	// Key event?
	switch (event->what) {
		case keyDown:
		case autoKey:
...
// Identify keypresses that should invoke the Cancel button.
static Boolean is_cancel_key_event(EventRecord *event) {
	long key_info, kchr_id, state;
	Handle kchr_rsrc = nil;
	Ptr kchr_data = nil;
	short virtual_key, key_code;
	char char_code;
	...
	// Escape key?
	char_code = event->message & charCodeMask;
	if (k_escape_key == char_code) return true;

	// Command key?
	if (!(event->modifiers & cmdKey)) return false;

	// Some international keyboards require the Shift key to type a period.
	// Technote TE 23: International Canceling
	if (g_has_script_manager) {
		// Try to use the KCHR cache, available in System 7 and later.
		kchr_data = (Ptr)GetEnvirons(smKCHRCache);

		// If no KCHR cache, try to load the resource.
		if (nil == kchr_data) {
			kchr_id = GetScript(GetEnvirons(smKeyScript), smScriptKeys);
			kchr_rsrc = GetResource('KCHR', kchr_id);
			kchr_data = *kchr_rsrc;
		}
	}
	...
	// Got KCHR data?
	if (nil != kchr_data) {
		virtual_key = (event->message & keyCodeMask) >> 8;

		// Combine virtual key code and all modifiers except Command key.
		key_code = virtual_key | (event->modifiers & k_modifiers_except_cmd_mask);

		// Translate this key code to character codes.
		state = 0;
		key_info = KeyTrans(kchr_data, key_code, &state);

		// Release KCHR resource if it was loaded.
		if (nil != kchr_rsrc) ReleaseResource(kchr_rsrc);

		// Check one of the two possible char codes generated by this key.
		char_code = key_info & charCodeMask;
		if ('.' == char_code) return true;

		// Check the other char code.
		char_code = (key_info >> 16) & charCodeMask;
	}

	return '.' == char_code;
}

// Intercept dialog box event handling.
static pascal Boolean dialog_filter(DialogPtr dialog, EventRecord *event, short *item) {
	...
	// Keyboard equivalents for OK and Cancel buttons.
	if (is_ok_key_event(event)) {
		key_item = ok;
	} else if (is_cancel_key_event(event)) {
		// If no Cancel button exists, Cancel keyboard equiv invokes OK button.
		GetDItem(dialog, cancel, &kind, &handle, &rect);
		key_item = ((ctrlItem | btnCtrl) == kind) ? cancel : ok;
	} else {
		key_item = 0;
	}

	if (0 != key_item) {
		flash_button(dialog, key_item);
		*item = key_item;
		return true;
	}
	...
	switch (event->what) {
		...
		case updateEvt:
			if ((DialogPtr)event->message == dialog) {
				draw_default_button_outline(dialog);
			} else {
...
static void do_about(void) {
	Alert(r_ALRT_about, dialog_filter);
}
